...
2. Expand the **Mergin Maps** entry in the Browser panel to show projects in your workspace:
   ![Mergin Maps projects in QGIS Browser](./qgis-mergin-my-projects.webp "Mergin Maps projects in QGIS Browser")
...
4. **Right-click** it and select **Open QGIS project**:
   ![Open Mergin Maps project in QGIS](./qgis-mergin-open-project.webp "Open Mergin Maps project in QGIS")
...
If this is the case, click on the **Synchronise** icon from the <QGISPluginName /> toolbar to synchronise the changes
   ![Mergin Maps QGIS plugin synchronisation icon](../../manage/plugin/plugin-toolbar-sync.webp "Mergin Maps QGIS plugin synchronisation icon")
...
1. **Double-click** the `hedges` layer in QGIS to open its **Layer properties**
   ![Hedges layer in QGIS](./qgis-hedges-layer.webp "Hedges layer in QGIS")

2. Select the **Symbology tab** on the left and click the current colour to change it:
   ![Layer properties Layer Symbology](./qgis-symbology-colour.webp "Layer properties Layer Symbology")

3. **Pick a new colour** for hedges which stands out better
   ![Select colour for layer symbology](./qgis-select-colour-dialog.webp "Select colour for layer symbology")

4. Click **OK**
5. Increase the **line width** to 0.46mm and click **OK**
   ![Set line width in Layer symbology](./qgis-symbology-width.webp "Set line width in Layer symbology")
...
1. In QGIS, save your project:
   ![Save QGIS project](./qgis-save-project.webp "Save QGIS project")

2. Use the **Synchronise Mergin Maps Project** button:
   ![Synchronise Mergin Maps Project](../../manage/plugin/plugin-toolbar-sync.webp "Synchronise Mergin Maps Project")

   The **Project status** window will open with the overview of local changes. Click **Sync** to synchronise the project:
   ![Project status with local changes](./qgis-project-status.webp "Project status with local changes")
   
   QGIS and your project in the cloud should now be synchronised.
   ![Successful synchronisation message](./qgis-mergin-project-sync.webp "Successful synchronisation message")
...
4. On the **Home** tab, you should see that the `trees-and-hedges` project has *Pending changes to synchronise*.
   ...
   ![Updated project in Mergin Maps mobile app](./merginmaps-mobile-project-with-pending-changes.webp "Updated project in Mergin Maps mobile app")
...
5. Tap the project to open it. 
   ...
   ![Layer with new symbology in Mergin Maps mobile app](./merginmaps-mobile-better-line-style.webp "Layer with new symbology in Mergin Maps mobile app")
...
![Feature with label in QGIS](./qgis-labels-example.webp "Feature with label in QGIS")

1. Double-click the `trees` layer in QGIS:
   ![Trees layer in QGIS Layer panel](./qgis-trees-layer.webp "Trees layer in QGIS Layer panel")
...
3. Set the labelling mode to **Single Labels**:
   ![QGIS Labels settings in Layer properties](./qgis-symbology-labels.webp "QGIS Labels settings in Layer properties")

4. Set the **Value** to the `species` attribute
   ![Choose single label attribute](./qgis-symbology-labels-2.webp "Choose single label attribute")

5. Enable **Draw text buffer** under **Buffer** settings and click **OK**
   ![Enable Draw text buffer](./qgis-symbology-labels-3.webp "Enable Draw text buffer")
...
6. Test out these settings in the <MobileAppNameShort /> as described in the [Trialling changes](#trialling-changes-in-mergin-maps-mobile-app) section above.
   ![Layers with labels in Mergin Maps mobile app](./merginmaps-mobile-labels.webp "Layers with labels in Mergin Maps mobile app")
...
![Feature preview panel in Mergin Maps mobile app](./merginmaps-mobile-preview-panel-changes.webp "Feature preview panel in Mergin Maps mobile app")
...
3. Set the **Display Name** to `species`:
   ![Display name attribute in Layer properties](./qgis-display-name.webp "Display name attribute")
...
4. Set the HTML map tip to:
   ...
   condition
   ```
   ![HTML map tip in Layer properties](./qgis-html-map-tip.webp "HTML map tip in Layer properties")
...
![Layers in Mergin Maps mobile app](../../field/mobile-app-ui/mobile-app-layers.webp "Layers in Mergin Maps mobile app")
...
1. In QGIS, click the **Manage Map Themes** button and select **Add Theme...**:
   ![QGIS Add map theme](./qgis-add-theme.webp "QGIS Add map theme")

2. Call the theme **All layers** and click **OK**
   ![Name the new theme](./qgis-name-theme.webp "Name the new theme")

3. **Uncheck** the trees and Survey notes layers:
   ![Uncheck layers that should not be visible](./qgis-toggle-layers.webp "Uncheck layers that should not be visible")

4. Add another theme like in step 1, calling it **Hedges** and click **OK**:
   ![Create map theme](./qgis-name-theme-2.webp "Create map theme")

   Switching between the two themes in QGIS causes the layer visibility to be updated accordingly.
   ![QGIS switching map themes](./qgis-toggle-themes.webp "QGIS switching map themes")
...
5. Test out these settings in the <MobileAppNameShort /> as described in the [Trialling changes](#trialling-changes-in-mergin-maps-mobile-app) section above
   ...
   ![Map themes in Mergin Maps mobile app](./merginmaps-mobile-map-themes-button.webp "Map themes in Mergin Maps mobile app")
    
    Simply switch between map themes in your project by tapping the one you want to use.
   ![Switch map themes in Mergin Maps mobile app](./merginmaps-mobile-map-themes.webp "Switch map themes in Mergin Maps mobile app")
...
![Mergin Maps mobile app default map extent](../capturing-first-data/merginmaps-mobile-location-shown-on-osm.webp "Mergin Maps mobile app default map extent")
...
1. In QGIS, zoom / pan the map to your desired default extent:
   ![Desired project extent in QGIS](./qgis-desired-project-extent.webp "Desired project extent in QGIS")

2. Select **Project > Properties...**
   ![QGIS Project properties](./qgis-project-properties.webp "QGIS Project properties")

3. Select the **View Settings tab**
   ![View settings in Project properties](./qgis-project-properties-project-extent.webp "View settings in Project properties")

4. Check **Set Project Full Extent**
5. Click **Map Canvas Extent**:
   ![Use map canvas extent as project full extent](./qgis-project-properties-project-extent-set-by-canvas.webp "Use map canvas extent as project full extent")
...
7. Test out these settings in the <MobileAppNameShort /> as described in the [Trialling changes](#trialling-changes-in-mergin-maps-mobile-app) section above
   ...
   ![Zoom to project in Mergin Maps mobile app](./merginmaps-mobile-zoom-to-project-button.webp "Zoom to project in Mergin Maps mobile app")
