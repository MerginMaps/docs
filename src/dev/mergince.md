...
Provided that `docker` and `docker-compose` are installed on your host, running Mergin Maps stack should be as simple as running `docker-compose`. However, before doing that you would need to [configure](#configure) your server setup via environment variables in [.prod.env](https://github.com/MerginMaps/mergin/blob/master/.prod.env) file. 
...
​​
### Initialize
If server is started for the first time, database needs to be initialized and super-user created (set admin username, password and email):
```shell
...
​
### Configure
There are several application settings which can be changed via [config variables](https://github.com/MerginMaps/mergin/blob/master/.prod.env). Some of them have defaults and some of them needs to be modified, particularly those with "fixme" placeholder (marked with asterisks below).
​
#### Server basics
...
| `SERVICE_ID`             | string    |           | Deployment UUID. Autogenerated on first run. |
​
...
| Variable name            | Type      | Default   | Description |
|--------------------------|-----------|-----------|-------------|
| `SECRET_KEY`*            | string    |           | Secret key for auth, should be generated strong string. |
| `SECURITY_PASSWORD_SALT`*| string    |           | Password salt for hashing, should be generated strong string. |
| `BEARER_TOKEN_EXPIRATION`| integer   |  `43200`  | Lifetime of auth bearer token in seconds. When expired, user needs to log in again. |
...
| `DB_USER`*                | string    |`postgres` | Postgres' user to connect to <MainPlatformName /> database.  |
| `DB_PASSWORD`*            | string    |`postgres` | Postgres' user password.  |
...
| `FILE_EXPIRATION`         | integer | `172800`    | When geopackage file was updated with "diffable" change, original data are being removed (as they can be reconstructed on demand) to save disk space. File lifetime in seconds.    |
...
|`CELERY_RESULT_BACKEND`|string|`redis://merginmaps-redis:6379/0`| Connection details to celery result backend broker. If non-default, it should match definition in `docker-compose` file.  |
​
​
