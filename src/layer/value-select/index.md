...
|:---:|:---:|
|[Drop-down menu with predefined values](#value-map) |![Mergin Maps mobile app value map field form](./mobile-form-valuemap-preview.webp "Mergin Maps mobile app value map field form")   |
|[Drop-down menu with values from another table, allowed multiple selection](#value-relation) |![Mergin Maps mobile app value relation field form](./mobile-form-value-relation-preview.webp "Mergin Maps mobile app value relation field form")   |
...
![QGIS value map field form](./qgis-form-valuemap.webp "QGIS value map field form")

Now you can select the value from a drop-down menu in the <MobileAppNameShort />:
![Mergin Maps mobile app value map field form](./mobile-form-valuemap.webp "Mergin Maps mobile app value map field form")
...
![QGIS value relation field form](./qgis-form-value-relation-multiselect.webp "QGIS value relation field form")
...
![Mergin Maps mobile app value relation multiple selection](./mobile-form-value-relation.webp "Mergin Maps mobile app value relation multiple selection")
...
![QGIS category value table](./qgis-table-habitat.webp "QGIS category value table")
...
![QGIS type value table](./qgis-table-type.webp "QGIS type value table")
...
![QGIS subtype value table](./qgis-table-subtype.webp "QGIS subtype value table")
...
2. The `habitat` field aliased as *Land use* is set up using the **Value relation** widget. Values are defined in the `plant-habitat` table:
   ...
   ![QGIS form value relation](./qgis-form-value-relation.webp "QGIS form value relation")
...
3. The `type` field (aliased as *Type*) uses the **Value relation** widget with values from the `plant-type` table:
   ...
   - **Filter expression**: `"habitat-code"= current_value('habitat')` is used to limit the options in the drop-down menu to values where the `habitat-code` of the value is the same as the current value of the `habitat` field.
    ![QGIS form value relation filter expression](./qgis-form-value-relation-expression.webp "QGIS form value relation filter expression")
...
4. Likewise, the `subtype` field (aliased as *Plant type*) uses the **Value relation** widget with values defined in the `plant-sub-type` table:
   ...
   - **Filter expression**: `"Code" = current_value('type')`
    ![QGIS form value relation filter expression](./qgis-form-value-relation-expression-subtype.webp "QGIS form value relation filter expression")
